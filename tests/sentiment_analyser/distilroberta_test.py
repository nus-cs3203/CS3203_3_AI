...
classifier =  DistilRobertaClassifier()
context = SentimentAnalysisContext(classifier)
df_result = context.analyze(df, text_cols=["title_with_desc"])
df_result = df_result[["title_with_desc_emotion", "title_with_desc_score"]]  
    
df_result.to_csv("tests/sentiment_analyser/data/2022_2025_merged_llm_pre_500_distilroberta.txt", index=False)